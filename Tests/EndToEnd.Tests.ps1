...
Describe "Invoke-AyfieInspector end-to-end" {
    It "produces a complete, error-free text report with every section, in the expected order" {
        ...
        $sectionOrder = @(
            ...
            "RESOURCE USAGE", "EXPIRATIONS AND CAPACITY DEPLETIONS", "FIREWALL OPENINGS",
            "AUTHENTICATION METHOD",
            "DATA SOURCE USER SYNCING", "SAGA INFO", "BACKUPS", "SAGA LICENSE INFO",
            "CUSTOM.ENV FILE CONTENT", "TEMPORARY .ENV FILE CHANGES",
            "SCHEDULED RESTART", "DATABASE INFO", "SUPERVISOR INFO", "PERSONAL ASSISTANT",
            "LINGO INFO", "SOLR INFO", "CUSTOM REFINERS",
            "DOCKER IMAGES CURRENTLY IN USE", "DATA SOURCE CONNECTIONS",
            "DATABASE CONNECTOR CONFIGURATIONS", "CUSTOM INDEX RULES",
            "CUSTOM SEARCH RULES"
        )
        $previousIndex = -1
